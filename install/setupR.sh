...
install.packages( c('argparser', 'S4Vectors', 'openxlsx', 'Matrix', 'tibble', 'statmod'), repos='http://cran.us.r-project.org', lib=Sys.getenv('R_LIBS_USER') ); \
...
biocLite( c('Seurat', 'SummarizedExperiment','zinbwave', 'BiocParallel', 'edgeR', 'limma','clusterExperiment' ), lib=Sys.getenv('R_LIBS_USER')  )"
	...
	Rscript -e "install.packages( c('argparser', 'S4Vectors', 'openxlsx', 'Matrix', 'tibble', 'statmod'), repos='http://cran.us.r-project.org' ); \
source('https://bioconductor.org/biocLite.R'); \
